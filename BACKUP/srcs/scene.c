...
/*   Updated: 2020/03/09 11:46:39 by amartin-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**fill_tmp_map(t_data *data, t_map *map, char **tmp, char *line)
{
	...
	i = 0;
	if (!(tmp = malloc(sizeof(char**) * (map->y))))
		error_exit(data, "Failed allocating memory for tmp map\n", "");
	while (i < map->y)
	{
		if (!(tmp[i] = malloc(sizeof(char*) * (map->x + 1))))
			error_exit(data, "Failed allocating memory for tmp map\n", "");
		if (i != (map->y - 1))
			tmp[i] = ft_strcharcpy(tmp[i], map->map[i], map->x, ' ');
		else
			tmp[i] = ft_strcharcpy(tmp[i], line, map->x, ' ');
		i++;
	}
	return (tmp);
}
...
void	fill_resolution(t_data *data, char *line, t_scene *scene)
{
	...
	while (ft_isdigit(line[i]))
		scene->win_Rx = (scene->win_Rx * 10) + (line[i++] - '0');
	while (line[i] == ' ')
		i++;
	while (ft_isdigit(line[i]))
		scene->win_Ry = (scene->win_Ry * 10) + (line[i++] - '0');
	while (line[i] == ' ')
		i++;
	if (line[i])
		error_exit(data, "Wrong resolution in .cub file\n", "");
	if (scene->win_Rx < 50 || scene->win_Ry < 50)
		error_exit(data, "Resolution is too small\n", "");
	if (scene->win_Rx > 2560)
		scene->win_Rx = 2560;
	if (scene->win_Ry > 1440)
		scene->win_Ry = 1440;
}
...
void	fill_texture(t_data *data, char *line, char *texture)
{
	...
	if (!ft_strncmp(texture, "NO", 2))
		data->scene.NO = ft_strdup(tmp);
	if (!ft_strncmp(texture, "SO", 2))
		data->scene.SO = ft_strdup(tmp);
	if (!ft_strncmp(texture, "WE", 2))
		data->scene.WE = ft_strdup(tmp);
	if (!ft_strncmp(texture, "EA", 2))
		data->scene.EA = ft_strdup(tmp);
	if (!ft_strncmp(texture, "S ", 2))
		data->scene.S = ft_strdup(tmp);
	free(tmp);
	tmp = NULL;
}
...
void	fill_settings(t_data *data, char *line, int i)
{
	if (line[i] == 'R' && line[i + 1] == ' ')
		fill_resolution(data, &line[i], &data->scene);
